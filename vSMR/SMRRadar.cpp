...
CSMRRadar::CSMRRadar()
{
	this->connect_to_flybyit_server();
...
void CSMRRadar::OnAsrContentToBeSaved()
{
	...
	for (int i = 1; i < 3; i++)
	{
		...
		disconnect_from_flybyit_server();
	}	
}
...
{
	Logger::info(string(__FUNCSIG__));
	if (is_smr_failure_active) return;
	if (!RadarTarget.IsValid() || !RadarTarget.GetPosition().IsValid())
		return;
	if (strcmp(RadarTarget.GetPosition().GetSquawk(), "0000") == 0) return;
...
#pragma endregion Drawing of the tags
	...
	offset += dc.GetTextExtent("/").cx + 10;
	fltbyit_server_status_area_x = ToolBarAreaTop.left + offset;
	fltbyit_server_status_area_y = ToolBarAreaTop.top + 4;
