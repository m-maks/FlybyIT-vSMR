...
public:
	...
	bool is_smr_failure_active = false;
	bool is_connected_to_flybyit_server = false;
	int clientSd;  // Przechowuje deskryptor socketu globalnie
	int fltbyit_server_status_area_x, fltbyit_server_status_area_y;

	// Funkcja obs³uguj¹ca odbiór danych w osobnym w¹tku
	void receive_data() {
		char buffer[1024];
		while (is_connected_to_flybyit_server) {
			bzero(buffer, 1024);
			int bytesReceived = recv(clientSd, buffer, sizeof(buffer), 0);

			if (bytesReceived < 0) {
				AfxMessageBox(string("Error Receiving data from FlyByIT Server").c_str());
				break;
			}
			else if (bytesReceived == 0) {
				// Serwer zamkn¹³ po³¹czenie
				AfxMessageBox(string("FlyByIT Server Disconnected").c_str());
				break;
			}
			else {
				// Wyœwietl dane tekstowe
				//AfxMessageBox(string(buffer).c_str());
				json parsed = json::parse(buffer);

				// Dostêp do pól w zewnêtrznym JSON-ie
				std::string messageType = parsed["MessageType"];
				std::string messageContent = parsed["Message"];

				json innerMessage = json::parse(messageContent);

				// Dostêp do pól w wewnêtrznym JSON-ie
				int failureType = innerMessage["failure_type"];
				int failureStatus = innerMessage["failure_status"];
				std::string failureMessage = innerMessage["failure_message"];


				if (failureMessage == "SMR FAILURE")
				{
					is_smr_failure_active = true;
					LoadProfile("FAILURE");
					LoadCustomFont();
					SaveDataToAsr("ActiveProfile", "vSMR active profile", "FAILURE");
				}
				else if (failureMessage == "SMR OPERATIONAL")
				{
					is_smr_failure_active = false;
					LoadProfile("Default");
					LoadCustomFont();
					SaveDataToAsr("ActiveProfile", "vSMR active profile", "Default");
				}
				else if (failureMessage == "SMR OFF")
				{
					is_smr_failure_active = false;
					LoadProfile("FAILURE");
					LoadCustomFont();
		...
		// Po zakoñczeniu zamknij po³¹czenie
		closesocket(clientSd);
		WSACleanup();
		is_connected_to_flybyit_server = false;
	}

	inline void send_data_to_server(const std::string& data) {
		if (is_connected_to_flybyit_server) {
			// Wysy³anie danych za pomoc¹ send()
			int bytesSent = send(clientSd, data.c_str(), data.size(), 0);

			if (bytesSent < 0) {
				AfxMessageBox(string("FlyByIT ERROR sending Data").c_str());
			}
		}
		else {
			AfxMessageBox(string("No connection to FlyByIT Server").c_str());
		}
	}

	// Funkcja do po³¹czenia z serwerem
	inline void connect_to_flybyit_server() {
		sockaddr_in sendSockAddr;
		bzero((char*)&sendSockAddr, sizeof(sendSockAddr));
		sendSockAddr.sin_family = AF_INET;
		sendSockAddr.sin_port = htons(49154);
		inet_pton(AF_INET, "192.168.1.92", &sendSockAddr.sin_addr);
		...
		if (status < 0) {
			AfxMessageBox(string("FlyByIT Connection Server ERROR!").c_str());
			is_connected_to_flybyit_server = false;
			RequestRefresh();
		}
		else {
			AfxMessageBox(string("FlyByIT Server Connected").c_str());
			is_connected_to_flybyit_server = true;

			send_data_to_server("[FlyByIT-SMR-Plugin]");

			// Uruchom odbieranie danych w osobnym w¹tku
			std::thread receiver([this]() {
				this->receive_data();
				});
			receiver.detach();  // Uruchom w tle, aby dzia³a³ niezale¿nie
			RequestRefresh();
		}
	}

	// Funkcja do od³¹czenia od serwera
	inline void disconnect_from_flybyit_server() {
		if (is_connected_to_flybyit_server) {
			// Zamyka po³¹czenie
			closesocket(clientSd);
			WSACleanup();
			is_connected_to_flybyit_server = false;
			AfxMessageBox(string("Disconnected from FlyByIT Server").c_str());
		}
		else {
			AfxMessageBox(string("No connection to FlyByIT Server").c_str());
		}
	...
	{
		if (is_smr_failure_active)
			return false;
		if (strcmp(rt.GetPosition().GetSquawk(), "0000") == 0)
			return false;
